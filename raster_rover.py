...
def compute_mask(arr, mode, value, threshold=None):
...
VIEWER_CSS = r"""
  ...
  * { box-sizing: border-box; }
  ...
  .tagline { font-size: 12px; color: var(--muted); margin-bottom: 6px; }
  ...
  .panel { background: var(--panel); border-radius: 10px; padding: 14px 16px; min-width: 250px; }
  canvas { background: #0c0d10; border-radius: 10px; display: block; cursor: crosshair; }
  #viewStack { position: relative; }
  #mapContainer {
    display: none; border-radius: 10px; background: #0c0d10;
    width: 720px; max-width: 90vw; height: 640px;
  }
...
function applyCriteria(mode, value, threshold) {
  let count = 0;
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const v = values[r][c];
      ...
      let hit = false;
      if (v !== null) {
        if (mode === 'within') hit = Math.abs(v - value) <= threshold;
        else if (mode === 'above') hit = v >= value;
        else if (mode === 'below') hit = v <= value;
      }
...
function syncThresholdVisibility() {
  thresholdRow.style.display = (modeSelect.value === 'within') ? 'block' : 'none';
}
modeSelect.addEventListener('change', syncThresholdVisibility);
...
document.getElementById('selectBtn').addEventListener('click', () => {
  ...
  lastCriteria = crit;
  const count = applyCriteria(crit.mode, crit.value, crit.threshold);
...
function buildSelectionOverlayDataUrl(crit) {
  ...
  for (let r = 0; r < h; r++) {
    for (let c = 0; c < w; c++) {
      ...
      let hit = false;
      if (v !== null) {
        if (crit.mode === 'within') hit = Math.abs(v - crit.value) <= crit.threshold;
        else if (crit.mode === 'above') hit = v >= crit.value;
        else if (crit.mode === 'below') hit = v <= crit.value;
      }
      const p = (r * w + c) * 4;
      if (hit) {
...
async function doExport(kind, btn) {
  ...
  try {
    const resp = await fetch('/export', {
      ...
      body: JSON.stringify({ mode: crit.mode, value: crit.value, threshold: crit.threshold, kind, geometry })
    });
    if (!resp.ok) {
...
<div class="layout">
  ...
  <div style="display:flex; flex-direction:column; gap:16px;">
    ...
    <div class="panel">
      ...
      <div class="hint">
        Exports use the Mode/Value/Threshold above, whether or not you've clicked Select.
      </div>
    </div>
...
def create_app(initial_arr, initial_transform, initial_crs_wkt, initial_crs_display,
    ...
    def export():
        ...
        if mode not in ("within", "above", "below"):
            return jsonify({"error": "mode must be 'within', 'above', or 'below'"}), 400
        if value is None:
        ...
        try:
            mask = compute_mask(arr, mode, value, threshold)
